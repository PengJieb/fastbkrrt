...
bool CollisionDetector::isThisPointCollides(float wx, float wy) {
  ...
  {
    // ROS_INFO("in wx=%f, wy=%f, something went wrong ",wx,wy);
    // ROS_INFO("here mx=%d, my=%d ",mx,my);
    return true;
  }
  ...
  if (cost > 0)
  {
	// ROS_INFO("in mx=%d, my=%d, get cost %d ",mx,my,cost);
	return true;
  }
...
}  // namespace rrt_star_local_planner
